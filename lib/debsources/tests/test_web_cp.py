...
class CopyrightTestCase(DebsourcesBaseWebTests, unittest.TestCase):
    ...
    def test_api_packages_list(self):
        rv = json.loads(self.app.get("/copyright/api/list/").data)
        self.assertIn({"name": "ocaml-curses"}, rv["packages"])
        self.assertEqual(len(rv["packages"]), 19)

    def test_api_by_prefix(self):
        rv = json.loads(self.app.get("/copyright/api/prefix/o/").data)
        self.assertIn({"name": "ocaml-curses"}, rv["packages"])
        # suite specified
        rv = json.loads(self.app.get("/copyright/api/prefix/o/?suite=wheezy").data)
        self.assertIn({"name": "ocaml-curses"}, rv["packages"])
        # a non-existing suite specified
        rv = json.loads(
            self.app.get("/copyright/api/prefix/libc/?suite=non-existing").data
        )
        self.assertEqual([], rv["packages"])
        # special suite name "all" is specified
        rv = json.loads(self.app.get("/copyright/api/prefix/libc/?suite=all").data)
        self.assertIn({"name": "libcaca"}, rv["packages"])

    def test_by_prefix(self):
        rv = self.app.get("/copyright/prefix/libc/")
        self.assertIn(b"/license/libcaca", rv.data)
        # suite specified
        rv = self.app.get("/copyright/prefix/libc/?suite=squeeze")
        self.assertIn(b"/license/libcaca", rv.data)
        # a non-existing suite specified
        rv = self.app.get("/copyright/prefix/libc/?suite=non-existing")
        self.assertNotIn(b"/license/libcaca", rv.data)
        # special suite name "all" is specified
        rv = self.app.get("/copyright/prefix/libc/?suite=all")
        self.assertIn(b"/license/libcaca", rv.data)

    def test_latest(self):
        rv = self.app.get("/copyright/license/gnubg/latest/", follow_redirects=True)
    ...
    def test_non_machine_readable_license(self):
        ...
        self.assertNotIn(b"<div class='r_files' id='copyright_info'>", rv.data)

    def test_machine_readable_license(self):
        rv = self.app.get("/copyright/license/gnubg/1.02.000-2/")
        self.assertIn(b"<div class='r_files' id='copyright_info'>", rv.data)

    def test_common_licenses_link(self):
        rv = self.app.get("/copyright/license/gnubg/1.02.000-2/")
        self.assertIn(b"/src/base-files/latest/licenses/GFDL-1.3", rv.data)
    ...
    def test_api_checksum(self):
        ...
        self.assertEqual(rv["result"]["copyright"][1]["license"], "BSD")
        self.assertEqual(rv["result"]["copyright"][2]["package"], "bsdgames-nonfree")
        self.assertEqual(rv["result"]["copyright"][2]["version"], "2.17-6")
        self.assertEqual(rv["result"]["copyright"][2]["path"], "COPYING")
        ...
        rv = json.loads(
            self.app.get(
                ...
                "aefa1c0a19d8ba3cf601d0e8a706b4cfa9661a6b8a"
            ).data
        )

        # debian/* under gpl2
        self.assertEqual(rv["result"]["copyright"][6]["license"], "GPL-2+")
        # /* under gpl2
        self.assertEqual(rv["result"]["copyright"][8]["license"], "GPL-2+")
        ...
        rv = json.loads(
            self.app.get(
                "/copyright/api/sha256/"
                "?checksum=2e6d31a5983a91251bfae5"
                "aefa1c0a19d8ba3cf601d0e8a71a6b8a"
            ).data
        )
    ...
    def test_api_package_filter_checksum(self):
        rv = json.loads(
            self.app.get(
                ...
                "6b4cfa9661a6b8a&package=gnubg"
            ).data
        )
        self.assertEqual(rv["result"]["copyright"][0]["package"], "gnubg")
        self.assertNotEqual(
            rv["result"]["copyright"][0]["version"],
    ...
    def test_api_suite_filter_checksum(self):
        ...
        self.assertGreaterEqual(
            len(rv2["result"]["copyright"]), len(rv["result"]["copyright"])
        )
    ...
    def test_api_checksum_latest(self):
        ...
        self.assertLessEqual(
            len(rv["result"]["copyright"]), len(rv2["result"]["copyright"])
        )

    def test_checksum_view(self):
        rv = self.app.get(
            "/copyright/sha256/?checksum="
            "2e6d31a5983a91251bfae5aefa1c0a"
            "19d8ba3cf601d0e8a706b4cfa9661a6b8a"
        )
        self.assertIn(b"This checksum appears 12 times", rv.data)
        self.assertIn(b"Most frequent license is: <i>GPL-2+</i>", rv.data)
    ...
    def test_checksum_view_package_filter(self):
        ...
        self.assertIn(b"This checksum appears 2 times", rv.data)
        self.assertNotIn(b"Most common package", rv.data)
    ...
    def test_checksum_view_suite_filter(self):
        ...
        self.assertIn(b"This checksum appears 8 times", rv.data)
        rv = self.app.get(
            "/copyright/sha256/?checksum="
            "2e6d31a5983a91251bfae5aefa1c0a"
            "19d8ba3cf601d0e8a706b4cfa9661a6b8a&suite=jessie"
        )
        self.assertIn(b"This checksum appears 7 times", rv.data)
    ...
    def test_checksum_view_one_result(self):
        ...
        self.assertIn(b"This checksum appears 1 time", rv.data)
        self.assertNotIn(b"<h3>Details</h3>", rv.data)
    ...
    def test_checksum_404(self):
        ...
        self.assertIn(b"0 results", rv.data)
    ...
    def test_api_search_filename_package(self):
        ...
        rv = json.loads(
            self.app.get("/copyright/api/file/gnubg/1.02.000-2/Makefile.am/").data
        )
        ...
        rv = json.loads(
            self.app.get(
                "/copyright/api/file/gnubg/0.90+20120429-1/" "doc/gnubg/gnubg.html/"
            ).data
        )
    ...
    def test_api_search_filename_suite_filter(self):
        ...
        self.assertEqual(rv["result"][0]["copyright"]["version"], "0.90+20120429-1")
        rv = json.loads(
            self.app.get(
        ...
        self.assertEqual(rv["result"][0]["copyright"]["version"], "0.90+20091206-4")
    ...
    def test_api_search_filename_latest(self):
        ...
        self.assertEqual(rv["result"][0]["copyright"]["version"], "1.02.000-2")

    def test_api_search_filename_all(self):
        rv = json.loads(
            self.app.get("/copyright/api/file/gnubg/all/doc/gnubg/gnubg.html/").data
        )
    ...
    def test_batch_api(self):
        data = {
            "checksums": [
                "2e6d31a5983a91251bfae5aefa1c0a19d8ba3cf601d0e" "8a706b4cfa9661a6b8a",
                "4f721b8e5b0add185d6af7a93e577638d25eaa5c34129" "7d95b4a27b7635b4d3f",
                "sha_does_not_exist",
            ]
        }
        rv = json.loads(self.app.post("/copyright/api/sha256/", data=data).data)
        self.assertEqual(
            "2e6d31a5983a91251bfae5aefa1c0a19d8ba3cf601d0e" "8a706b4cfa9661a6b8a",
            rv["result"][0]["checksum"],
        )
    ...
    def test_search_filename_all(self):
        rv = self.app.get("/copyright/file/gnubg/all/doc/gnubg/gnubg.html/").data
        self.assertIn(b"File name appears 3", rv)

    def test_search_filename_suite_non_existing(self):
        rv = self.app.get("/copyright/file/gnubg/not/doc/gnubg/gnubg.html/").data
        self.assertIn(b"not found", rv)

    def test_search_filename_suite_filter(self):
        rv = self.app.get(
            "/copyright/file/gnubg/jessie/doc/gnubg/gnubg.html/", follow_redirects=True
        )
        self.assertNotIn(b"File name appears", rv.data)
    ...
    def test_search_filename(self):
        ...
        self.assertIn(b"GFDL-1.3+", rv.data)

    def test_synopsis_parsing(self):
        rv = self.app.get("/copyright/license/gnubg/1.02.000-2/")
        self.assertIn(b'<a href="#license-0">FSF-configure</a>', rv.data)
        # Test separating by ',' and, or and create correct links
        synopsis = (
            b'<a href="#license-0">FSF-configure</a>, and  '
            b'<a href="http://opensource.org/licenses/GPL-2.0">GPL-2+'
            b' with Libtool exception</a> or  <a href="http://opensou'
            b'rce.org/licenses/GPL-3.0">GPL-3+</a>'
        )
        self.assertIn(synopsis, rv.data)

    def test_glob_links(self):
        rv = self.app.get("/copyright/license/gnubg/1.02.000-2/")
        self.assertIn(b'<a href="/src/gnubg/1.02.000-2/fonts">fonts/*.ttf</a>', rv.data)
        self.assertIn(b'<a href="/src/gnubg/1.02.000-2/">*</a>', rv.data)
