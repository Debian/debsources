...
class ChecksumLicenseView(ChecksumView):
    def _get_files(self, checksum, package, suite):
        if suite != "latest":
            files = ChecksumView._files_with_sum(checksum, None, package, suite)
        else:
            files = ChecksumView._files_with_sum(checksum, slice_=None, package=package)
            ...
            for package in dd:
                version = sorted(
                    [item["version"] for item in dd[package]],
                    key=cmp_to_key(version_compare),
                )[-1]
                files.append(
                    list(filter(lambda f: f["version"] == version, dd[package]))[0]
                )
        return files
    ...
    def batch_api(self, checksums, package=None, suite=None):
        ...
        for sha in checksums:
            ...
            if not files:
                results.append(dict(checksum=sha, copyright=[], count=0))
            else:
                results.append(
                    dict(
                        count=len(files),
    ...
    def get_objects(self, **kwargs):
        ...
        if "api" in request.endpoint:
            ...
            if not d_copyright:
                return dict(
                    return_code=404, error="Checksum not found", count=0, result=[]
                )
            return dict(
                return_code=200,
                count=len(d_copyright),
                result=dict(checksum=checksum, copyright=d_copyright),
            )
        else:
            ...
            counter = Counter(
                [x["license"] for x in all_d_copyright if x["license"] is not None]
            ).most_common()
    ...
    def get_objects(self, packagename, version, path_to):
        ...
        if "api" in request.endpoint:
            ...
            return dict(
                ...
                count=len(files),
                result=[
                    dict(checksum=res.checksum, copyright=self._license_of_files(res))
                    for res in files
                ],
            )
        else:
...
class StatsView(GeneralView):
    ...
    def get_stats(self):
        ...
        dual_licenses = [
            ...
            for license in dual_res.keys()
            if "overall." in license
        ]

        licenses = [
            license.replace("overall.", "")
            for license in res.keys()
            if "overall." in license
        ]
